...
public class UserConfig {
	...
	@Bean("fake_user")
	@Scope("prototype")
	public User user_fake() {
		return User.builder().username(Faker.instance().name().username()).name(Faker.instance().name().name())
				.lastname(Faker.instance().name().lastName()).email(Faker.instance().internet().emailAddress()).build();
	}
